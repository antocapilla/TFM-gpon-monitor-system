...
class Ray(Visualizable):
    ...
    def __repr__(self):
        return f"Ray('{self.start_point}', {self.end_point})"
    ...
    def visualize(self, display, scale):
        ...
        if first_point and last_point is not None:
            start_point_scaled = (int(first_point[0] * scale[0]), int(first_point[1] * scale[1]))
            end_point_scaled = (int(last_point[0] * scale[0]), int(last_point[1] * scale[1]))
...
class Simulator:
    ...
    def propagate_ray(self, ray):
        ...
        if collision is not None:
            ...
            if ray.num_reflections == 0: # Caso de LOS
                self.refract_ray(ray, collision)
            elif ray.num_reflections >= self.max_reflections or ray.num_transmissions >= self.max_transmissions:
            ...
            raise Exception("Collision point not found")
            pass

    def find_closest_collision(self, ray):
        obstacles = self.environment.get_obstacles()
        if(ray.start_point is None):
            raise Exception("No start_point in Ray")
    ...
    def refract_ray(self, ray, collision):
        ...
        ray.start_point = collision.point # Caso de LOS
        self.propagate_ray(ray)
...
def create_random_walls(width, height, num_walls=10):
    walls = []
    material = Material(2.8, 0.0001, 5)
    min_wall_distance = int(width/6)
    min_wall_height = int(height/6)
...
if __name__ == "__main__":
    ...
    tx_antenna = Antenna(location=(10, 10), tx_power=10, radiation_pattern=None, frequency=2.4e9)

    # Crear la cuadrícula de receptores
    rx_grid = ReceiverGrid(dimensions=(width, height), resolution=50)

    # Crear el simulador
    simulator = Simulator(environment, tx_antenna, rx_grid, num_rays=1, max_reflections=1, max_transmissions=3)
